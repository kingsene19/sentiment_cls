...
if __name__ == "__main__":
    ...
    gru = CustomGRU(len(vocab), 512, 128, len(np.unique(train_emotions)))
    history = gru.fit(train_loader, val_loader, 10, lr=0.001)

    # Affichage des statistiques d'entrainement
    plot_train_statistics(history)

    # Evaluation du modèle
    gru.evaluate(test_loader, le)

    # Sauvegarde du modèle
    gru.save("initial_gru.pth")