...
class CustomLSTM(nn.Module):
    def __init__(self, input_size, emb_size, hidden_size, output_size):
        ...
        # Ajout de notre couche d'embedding
        self.i2e = nn.Linear(input_size, emb_size)
        
        # Paramètres pour les portes LSTM (input, forget, output) et cell state

        # Input Gate
        self.Wi = nn.Parameter(torch.Tensor(emb_size, hidden_size))  # Poids pour l'entrée (emb -> gru)
        self.Ui = nn.Parameter(torch.Tensor(hidden_size, hidden_size))  # Poids pour l'état caché (hidden -> gru)
        self.bi = nn.Parameter(torch.Tensor(hidden_size))  # Biais

        # Forget Gate
        self.Wf = nn.Parameter(torch.Tensor(emb_size, hidden_size))  # Poids pour l'entrée (emb -> gru)
        self.Uf = nn.Parameter(torch.Tensor(hidden_size, hidden_size))  # Poids pour l'état caché (hidden -> gru)
        self.bf = nn.Parameter(torch.Tensor(hidden_size))  # Biais

        # Output Gate
        self.Wo = nn.Parameter(torch.Tensor(emb_size, hidden_size))   # Poids pour l'entrée (emb -> gru)
        self.Uo = nn.Parameter(torch.Tensor(hidden_size, hidden_size)) # Poids pour l'état caché (hidden -> gru)
        self.bo = nn.Parameter(torch.Tensor(hidden_size)) # Biais

        # Cell state
        self.Wc = nn.Parameter(torch.Tensor(emb_size, hidden_size))   # Poids pour l'entrée (emb -> gru)
        self.Uc = nn.Parameter(torch.Tensor(hidden_size, hidden_size)) # Poids pour l'état caché (hidden -> gru)
        self.bc = nn.Parameter(torch.Tensor(hidden_size)) # Biais
    ...
    def forward(self, x):
        ...
        seq_len, bs, _ = x.size()
        # Initialisation de l'état caché et de la cellule à zéro
        ...
        for t in range(seq_len):
            ...
            # Calcul des valeurs des portes : input, forget, cell et output
            # Input gate -> importance de la nouvelle information
            i_t = torch.sigmoid(e_t @ self.Wi + h_t @ self.Ui + self.bi)
            # Forget gate -> quantité d'information à converser de l'état précédent
            f_t = torch.sigmoid(e_t @ self.Wf + h_t @ self.Uf + self.bf)
            # Cell state -> candidate pour la mise à jour de la cellule
            g_t = torch.tanh(e_t @ self.Wc + h_t @ self.Uc + self.bc)
            # Output gate -> importance de l'état de la cellule pour la sortie
            o_t = torch.sigmoid(e_t @ self.Wo + h_t @ self.Uo + self.bo)
            
            # Mise à jour de l'état de la cell (combinaison des contributions de la forget gate et de l'input gate)
            c_t = f_t * c_t + i_t * g_t
            # Mise à jour de l'état caché en appliquant l'output gate à l'état de la cell mis à jour
